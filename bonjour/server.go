...
func Register(instance, service, domain string, port int, hostname string, ipv4 string, text []string, iface *net.Interface) (*Server, error) {
	entry := NewServiceEntry(instance, service, domain)
	entry.Port = port
	entry.Text = text
	entry.HostName = hostname
	entry.AddrIPv4 = net.ParseIP(ipv4)
	...
	if ipv4 == "" {
		return nil, fmt.Errorf("Missing ipaddr")
	}
